...
def model_mk_rnnrff(input_dimensions, rnn_dim, rff_dim, n_classes):
    ...
    if len(inputs) == 1:
        ...
        for sz, d in sorted(input_dimensions.keys()):
            n_features = input_dimensions[sz, d]
            rnn_layers[sz, d] = SimpleRNN(units=rnn_dim, activation="tanh")
            rff_layers[sz, d] = RFFLayer(units=rff_dim)
            rffs = [rff_layers[sz, d](rnn_layers[sz, d](input_feature)) for input_feature in inputs[idx0:idx0+n_features]]
